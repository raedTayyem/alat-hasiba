...
/** Working Capital Calculator - Formula: Current Assets - Current Liabilities */

import { useState, useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import { DollarSign, Activity } from '@/utils/icons';
...
export default function WorkingCapitalCalculator() {
  ...
  const calculate = () => {
    ...
    if (isNaN(assets) || isNaN(liabilities)) { setError(t("errors.invalid_input", { ns: 'common' })); return; }
    setShowResult(false);
    setTimeout(() => {
      const workingCapital = assets - liabilities;
      const ratio = liabilities > 0 ? assets / liabilities : 0;
      setResult({ workingCapital, ratio });
      setShowResult(true);
    }, 300);
  };
  ...
  const inputSection = (
    <>
      <div className="text-2xl font-bold mb-6 text-center">{t("working_capital.title")}</div>
      <div className="max-w-md mx-auto space-y-4">
        <FormField label={t("working_capital.inputs.current_assets")} tooltip={t("working_capital.inputs.current_assets_tooltip")}>
          <NumberInput 
            value={currentAssets} 
            onValueChange={(val) => {setCurrentAssets(val.toString()); if(error) setError('');}}
            onKeyPress={handleKeyPress}
            placeholder={t("working_capital.inputs.current_assets")}
            startIcon={<DollarSign className="h-4 w-4" />}
            min={0}
          />
        </FormField>
        <FormField label={t("working_capital.inputs.current_liabilities")} tooltip={t("working_capital.inputs.current_liabilities_tooltip")}>
          <NumberInput 
            value={currentLiabilities} 
            onValueChange={(val) => {setCurrentLiabilities(val.toString()); if(error) setError('');}}
            onKeyPress={handleKeyPress}
            placeholder={t("working_capital.inputs.current_liabilities")}
            startIcon={<DollarSign className="h-4 w-4" />}
            min={0}
          />
        </FormField>
      ...
      {!result && <div className="mt-6 bg-card-bg border border-border rounded-xl p-6 max-w-2xl mx-auto">
        <p className="text-foreground-70">{t("working_capital.description")}</p>
      </div>}
    </>
  );
  ...
  const resultSection = result && showResult ? (
    <div className="bg-card-bg border border-border rounded-xl p-6 shadow-sm animate-fadeIn">
      <div className="text-center mb-6">
        <div className="text-foreground-70 mb-2">{t("working_capital.results.working_capital")}</div>
        <div className="text-4xl font-bold text-primary">${formatNumber(result.workingCapital)}</div>
        <div className="flex items-center justify-center mt-2 text-lg text-foreground-70">
          ...
          {t("working_capital.results.current_ratio")}: {formatNumber(result.ratio)}
        </div>
      </div>
    </div>
  ) : null;

  return <CalculatorLayout title={t("working_capital.title")} description={t("working_capital.description")}
