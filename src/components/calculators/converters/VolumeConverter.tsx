...
import { Calculator, ArrowRightLeft, Info, Beaker } from 'lucide-react';
...
interface UnitConversion {
  label: string;
  toLiters: number; // Convert to liters
}

export default function VolumeConverter() {
  const { t } = useTranslation(['calc/converters', 'common']);

  // Define all volume units with their conversion factors to liters
  const volumeUnits: Record<string, UnitConversion> = {
    liter: { label: t("volume.units.liter"), toLiters: 1 },
    milliliter: { label: t("volume.units.milliliter"), toLiters: 0.001 },
    gallon: { label: t("volume.units.gallon_us"), toLiters: 3.78541 },
    quart: { label: t("volume.units.quart_us"), toLiters: 0.946353 },
    pint: { label: t("volume.units.pint_us"), toLiters: 0.473176 },
    cup: { label: t("volume.units.cup_us"), toLiters: 0.236588 },
    fluidOunce: { label: t("volume.units.fluid_ounce_us"), toLiters: 0.0295735 },
    cubicMeter: { label: t("volume.units.cubic_meter"), toLiters: 1000 },
    cubicCm: { label: t("volume.units.cubic_centimeter"), toLiters: 0.001 },
  };
  ...
  useEffect(() => {
    ...
    if (!isNaN(numValue)) {
      convertVolume(numValue, fromUnit, toUnit);
    } else {
      setResult(0);
    }
  ...
  const convertVolume = (value: number, from: string, to: string) => {
    const valueInLiters = value * volumeUnits[from].toLiters;
    const convertedValue = valueInLiters / volumeUnits[to].toLiters;
  ...
  const unitOptions = Object.entries(volumeUnits).map(([key, unit]) => ({
    value: key,
    label: unit.label
  }));
          ...
          <div className="space-y-2">
            {recentConversions.map((conversion, index) => (
              <div key={index} className="p-3 bg-primary/5 rounded-md flex justify-between items-center text-sm">
                <span>
                  {(conversion.value).toFixed(2)} {volumeUnits[conversion.from].label}
                </span>
                <span className="text-primary">
                  <ArrowRightLeft className="h-4 w-4 inline mx-2" />
                </span>
                <span>
                  {(conversion.result).toFixed(2)} {volumeUnits[conversion.to].label}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 text-sm overflow-hidden rounded-md border border-border">
          ...
          <div className="grid grid-cols-3 gap-1 text-center p-2.5 odd:bg-muted/20">
            <div>1 {volumeUnits[fromUnit].label}</div>
            <div>=</div>
            <div>{(result / parseFloat(inputValue || '1')).toFixed(4)} {volumeUnits[toUnit].label}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-card p-3 rounded-lg border border-border">
            <div className="font-medium mb-1">{volumeUnits[fromUnit].label}</div>
            <div className="text-sm text-foreground-70">{t("common.source_unit")}</div>
          </div>
          <div className="bg-card p-3 rounded-lg border border-border">
            <div className="font-medium mb-1">{volumeUnits[toUnit].label}</div>
