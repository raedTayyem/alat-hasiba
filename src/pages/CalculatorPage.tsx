      ...
      >
        <ol className="flex flex-wrap items-center gap-1 text-sm">
          ...
          <li className="flex items-center">
            <Link
              to="/"
              className="flex items-center gap-1.5 text-muted-foreground hover:text-primary transition-colors duration-200 group focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded"
              ...
              {faqs.map((faq, index) => {
                ...
                const handleKeyDown = (e: React.KeyboardEvent) => {
                  ...
                  switch (e.key) {
                    ...
                    case 'End':
                      e.preventDefault();
                      (faqButtons[faqButtons.length - 1] as HTMLElement)?.focus();
                      break;
                  }
                };
                  ...
                  <div key={index} className="border border-border/50 rounded-xl overflow-hidden">
                    <h3>
                      <button
                        ...
                        className="w-full flex items-center justify-between p-4 text-start hover:bg-muted/30 transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-inset"
                        aria-expanded={isExpanded}
                        aria-controls={panelId}
                      >
                        <span className="font-medium pe-4">{faq.question}</span>
                        <ChevronDown className={`w-5 h-5 text-muted-foreground flex-shrink-0 transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} aria-hidden="true" />
                      </button>
                    </h3>
                    <div
                      id={panelId}
                      ...
                      hidden={!isExpanded}
                      className={isExpanded ? "px-4 pb-4 text-foreground-70 border-t border-border/50 pt-4 bg-muted/20" : ""}
                    >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4 text-start">
            <div className="bg-amber-100 dark:bg-amber-900/50 p-3 rounded-full">
              <svg className="w-8 h-8 text-amber-600 dark:text-amber-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          <a
            ...
            className="bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-8 rounded-xl transition-all shadow-lg shadow-amber-500/20 hover:-translate-y-1 flex items-center gap-2 whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2"
          >
            ...
            {relatedCalculators.map((calculator) => {
              ...
              return (
                <Link
                  ...
                  className="group bg-card hover:bg-card/80 border border-border hover:border-primary/30 rounded-2xl p-6 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                >
